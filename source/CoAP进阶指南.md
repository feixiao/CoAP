...
**实现注意：**
当一个CoAP服务端是由6LowPAN节点提供，当端口使用[[RFC4944]](http://tools.ietf.org/pdf/rfc4944)和[[RFC6282]](http://tools.ietf.org/pdf/rfc6282)定义的UDP端口压缩方式，采用61616-61632之间的端口号时，头部的压缩会更好。注意，如果某个服务端的UDP端口和默认端口不同，可以将它们（采用非默认端口的和采用默认端口的）视为两个不同的端。
